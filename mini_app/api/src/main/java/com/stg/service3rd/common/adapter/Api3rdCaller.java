...
public abstract class Api3rdCaller implements IApi3rdConfig, IApi3rdMethods, IApi3rdSimpleMethods {
    ...
    /***/
    @Override
    public <B> B post(IFunction func, HttpHeaders headers, Object payload, Class<B> responseType, @Nullable Map<String, ?> uriVariables) throws Exception {
        LocalDateTime startTime = LocalDateTime.now();

        String url = this.host3rd.getDomain() + func.getUri();
        URI uri = buildUrl(url, uriVariables);
        url = uri.toString();
        ...
        try {
            exchange = restTemplate.exchange(uri, HttpMethod.POST, requestEntity, responseType);
    ...
    public <B> B get(IFunction func, HttpHeaders headers, Object payload, Class<B> responseType, @Nullable Map<String, ?> uriVariables) throws Exception {
        ...
        URI uri = buildUrl(this.host3rd.getDomain() + func.getUri(), uriVariables);
        ...
        ResponseEntity<B> exchange = null;
        try {
            exchange = restTemplate.exchange(uri, HttpMethod.GET, requestEntity, responseType);
            return exchange.getBody();
        } catch (Exception ex) {
            exception = ex;
            throw ex;
    ...
    protected URI buildUrl(String baseUrl, Map<String, ?> uriVariables) {
