    ...
    public <B extends SoapResponseBody, R extends SoapResponse<B>> R post(IFunction func, HttpHeaders headers, SoapRequest payload, Class<R> responseType) throws Exception {
        ...
        try {
            ...
            if ("000".equals(responseStatus.getCode())) {
                ...
                throw new Api3rdException(StringUtils.isEmpty(responseStatus.getMessageEn()) ? responseStatus.getMessageVi() : responseStatus.getMessageEn(), new ErrorDto(exchange.getStatusCodeValue(), responseStatus.getCode(), responseStatus.getMessageVi()));
            }
        } catch (Exception ex) {
            exception = ex;
            throw ex;
