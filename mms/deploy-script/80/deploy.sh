...
[ ! -f ${TARGET}/config/application.properties ] && cp deploy-script/80/config/application.properties $TARGET/config/application.properties
...
sudo nohup java -jar -Dspring.profiles.active=prod -Duser.timezone=Asia/Seoul -Djava.io.tmpdir=$TMPDIR ${TARGET}/mms-1.0.0.jar -Xms512m -Xmx768m 1>${LOG_PATH}$PEFIX_LOG_NAME.log 2>&1 &
echo "MMS started.."
