...
public class MoldDownTimeEventRepositoryCustomImpl extends QuerydslRepositorySupport implements MoldDownTimeEventRepositoryCustom {
    public MoldDownTimeEventRepositoryCustomImpl() {
        super(MoldDowntimeEvent.class);
    }

    @Override
    public List<MoldDowntimeEvent> findMoldDownTimeEventOrderByOperatingStatus(Predicate predicate, Pageable pageable) {
        String[] properties = {""};
        ...
        QMold mold = moldDowntimeEvent.mold;
        JPQLQuery query = from(moldDowntimeEvent).where(predicate);
    ...
    @Override
    public List<MoldDowntimeEvent> findMoldDownTimeEventOrderByStatus(Predicate predicate, Pageable pageable) {
        String[] properties = {""};
        Sort.Direction[] directions = {Sort.Direction.DESC};
        pageable.getSort().forEach(order -> {
            properties[0] = order.getProperty();
        ...
        JPQLQuery query = from(moldDowntimeEvent)
                .leftJoin(mold).on(moldDowntimeEvent.moldId.eq(mold.id))
    ...
    public List<IdData> getAllIds(Predicate predicate) {
        QMoldDowntimeEvent table = QMoldDowntimeEvent.moldDowntimeEvent;
