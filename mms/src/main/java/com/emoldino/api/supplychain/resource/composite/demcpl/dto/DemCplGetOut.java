...
@NoArgsConstructor
public class DemCplGetOut extends ListOut<DemCplItem> {
	private List<DemCplChartItem> chartItems = new ArrayList<>();

	public DemCplGetOut(List<DemCplItem> content, List<DemCplChartItem> chartItems) {
		super(content);
		this.chartItems = chartItems;
	}

	public void addChartItem(DemCplChartItem chartItem) {
	...
	public List<Map<String, Object>> getChartItems2() {
		...
		DemCplChartItem firstItem = chartItems.get(0);
		for (int i = 1; i <= 50; i++) {
			...
			for (int j = 1; j <= len; j++) {
				DemCplChartItem item = chartItems.get(j - 1);
				String id = ValueUtils.toString(item.getPartId());
				if (ObjectUtils.isEmpty(id)) {
					id = ValueUtils.toString(item.getSupplierId());
				}
	...
	public static class DemCplItem {
		private Long partId;
	...
	@Builder
	@JsonInclude(Include.NON_NULL)
	public static class DemCplChartItem {
		private Long partId;
		private String partName;
		private String partCode;
