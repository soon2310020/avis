...
/**
 * Examp // Get your sourceSheet object, your code may be different
...
public class PoiCopySheet {
	...
	/**
	 * Copy data from a source sheet to a destination sheet.
	 * @param sourceSheet The source sheet from which data will be copied.
	 * @param destinationSheet The destination sheet where the data will be copied to.
	 */
	private static void copyDataValidations(Sheet sourceSheet, Sheet destinationSheet) {
		DataValidationHelper validationHelper = new XSSFDataValidationHelper((XSSFSheet) destinationSheet);
		List<? extends DataValidation> dataValidations = sourceSheet.getDataValidations();

		for (DataValidation dataValidation : dataValidations) {
			CellRangeAddressList addressList = dataValidation.getRegions();
			DataValidation newValidation = validationHelper.createValidation(
					dataValidation.getValidationConstraint(), addressList
			);
			newValidation.setShowErrorBox(dataValidation.getShowErrorBox());
			newValidation.setShowPromptBox(dataValidation.getShowPromptBox());
			newValidation.setErrorStyle(dataValidation.getErrorStyle());
			newValidation.setEmptyCellAllowed(dataValidation.getEmptyCellAllowed());
			newValidation.setSuppressDropDownArrow(dataValidation.getSuppressDropDownArrow());
			destinationSheet.addValidationData(newValidation);
		}
	}
	...
	private void copySheetInst() {
		...
		for (int i = srcSheet_.getFirstRowNum(); i <= srcSheet_.getLastRowNum(); i++) {
			...
			Row dstRow = dstSheet_.createRow(i);
			if (srcRow == null) {
				continue;
			}
	...
	private void copyRow(Row srcRow, Row dstRow) {
		...
		for (; j <= srcRow.getLastCellNum(); j++) {
			...
			if (srcCell != null) {
				...
				copyCell(srcCell, dstRow, dstCell);
				CellRangeAddress mergedRegion = getMergedRegion(srcSheet_, srcRow.getRowNum(), (short) srcCell.getColumnIndex());

				if (mergedRegion != null) {
					CellRangeAddress dstMergedRegion = new CellRangeAddress(mergedRegion.getFirstRow(), mergedRegion.getLastRow(), mergedRegion.getFirstColumn(),
	...
	private boolean isdstMergedRegion(CellRangeAddressWrapper dstMergedRegion) {
		return !mergedRegions_.contains(dstMergedRegion)
				&& !mergedRegions_.stream().filter(m -> m.range.formatAsString().equalsIgnoreCase(dstMergedRegion.range.formatAsString())).findFirst().isPresent();
	}

	private void copyCell(Cell srcCell, Row dstRow, Cell dstCell) {
		...
		copyComment(srcCell, dstRow, dstCell);

		switch (srcCell.getCellType()) {
		case STRING:
			RichTextString richTextSource = srcCell.getRichStringCellValue();
			dstCell.setCellValue(richTextSource);
			break;
		case NUMERIC:
	...
	private void copyComment(Cell srcCell, Row dstRow, Cell dstCell) {
		Comment srcComment = srcCell.getCellComment();
		if (srcComment != null && srcComment.getString() != null) {
			if (factory == null) {
				factory = dstWorkbook_.getCreationHelper();
			}
			if (drawing == null) {
				drawing = dstSheet_.createDrawingPatriarch();
			}
			ClientAnchor anchor = factory.createClientAnchor();
			anchor.setCol1(dstCell.getColumnIndex());
			anchor.setCol2(dstCell.getColumnIndex() + 5);
			anchor.setRow1(dstRow.getRowNum());
			anchor.setRow2(dstRow.getRowNum() + 70);
			Comment dstComment = drawing.createCellComment(anchor);
	...
	private CellStyle cloneCellStyle(CellStyle srcCellStyle) {
		...
		// new one
		Font dstFont = dstWorkbook_.findFont(srcFont.getBold(), srcFont.getColor(), srcFont.getFontHeight(), srcFont.getFontName(), srcFont.getItalic(), srcFont.getStrikeout(),
