package com.emoldino.api.common.resource.composite.alrdatapr;

import org.springframework.data.domain.Pageable;
import org.springframework.web.bind.annotation.GetMapping;
import org.springframework.web.bind.annotation.PostMapping;
import org.springframework.web.bind.annotation.RequestBody;
import org.springframework.web.bind.annotation.RequestMapping;

import com.emoldino.api.common.resource.base.note.dto.NoteIn;
import com.emoldino.api.common.resource.composite.alrdatapr.dto.AlrDatAprGetIn;
import com.emoldino.api.common.resource.composite.alrdatapr.dto.AlrDatAprGetOut;
import com.emoldino.framework.dto.BatchIn;
import com.emoldino.framework.dto.SuccessOut;

import io.swagger.annotations.Api;
import io.swagger.annotations.ApiOperation;

@Api(protocols = "http, https", tags = "Common / Alert / Data Approval")
@RequestMapping("/api/common/alr-dat-apr")
public interface AlrDatAprController {
	public static final String NAME = "Approval";
	public static final String NAME_PLURAL = "Approvals";

	@ApiOperation("Get " + NAME_PLURAL)
	@GetMapping
	AlrDatAprGetOut get(AlrDatAprGetIn input, Pageable pageable);

	@ApiOperation("Post " + NAME_PLURAL + " Note Batch")
	@PostMapping("/note-batch")
	SuccessOut postNoteBatch(AlrDatAprGetIn input, BatchIn batchin, @RequestBody NoteIn body);
}
