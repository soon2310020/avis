...
@Api(protocols = "http, https", tags = "Common / Part Config")
@RequestMapping("/api/common/par-stp")
public interface ParStpController {
	public static final String NAME = "Part";
	public static final String NAME_PLURAL = "Parts";

	@ApiOperation("Get " + NAME_PLURAL)
	@GetMapping
	ParStpGetOut get(ParStpGetIn input, Pageable pageable);
	...
	SuccessOut disableBatch(ParStpGetIn input, BatchIn batchin);

	@ApiOperation("Enable " + NAME_PLURAL + " Batch")
	@PutMapping("/enable-batch")
	SuccessOut enableBatch(ParStpGetIn input, BatchIn batchin);

	@ApiOperation("Post " + NAME_PLURAL + " Note Batch")
	@PostMapping("/note-batch")
	SuccessOut postNoteBatch(ParStpGetIn input, BatchIn batchin, @RequestBody NoteIn body);

	@ApiOperation("Export Static " + NAME_PLURAL)
	@GetMapping("/export")
	void export(ParStpGetIn input, BatchIn batchin, Pageable pageable, HttpServletResponse response) throws IOException;

	@ApiOperation("Move " + NAME_PLURAL + " Tab Items Batch")
	@PutMapping("/move-tab-items-batch")
	SuccessOut moveTabItemsBatch(ParStpGetIn input, BatchIn batchin, @RequestParam(name = "toTabName") String toTabName);

	@ApiOperation("Remove " + NAME_PLURAL + " Tab Items Batch")
	@DeleteMapping("/tab-items-batch")
	SuccessOut deleteTabItemsBatch(ParStpGetIn input, BatchIn batchin);
