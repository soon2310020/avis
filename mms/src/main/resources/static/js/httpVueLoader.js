(function umd(root, factory) {
  ...
  const addNoCacheLink = (srcLink) => {
    try {
      console.log("addNoCacheLink");
      var uri = URI.parse(srcLink);
      let params = parseParams(uri.query);
  ...
  Component.prototype = {
    ...
    _normalizeSection: function (eltCx) {
      ...
      if (eltCx === null || !eltCx.elt.hasAttribute("src")) {
        ...
        //false cache for load src
        let srcLink = eltCx.elt.getAttribute("src");
        if (
          srcLink &&
          eltCx.elt.hasAttribute("cus-no-cache") &&
          eltCx.elt.getAttribute("cus-no-cache") !== false
        ) {
          srcLink = addNoCacheLink(srcLink);
        }
        p = httpVueLoader.httpRequest(srcLink).then(function (content) {
