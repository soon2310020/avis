...
module.exports = {
  ...
  computed: {
    toolingList() {
      console.log(
        "toolingList",
        this.searchedTabList.filter((tab) => !tab.isDefaultTab)
      );
  ...
  methods: {
    ...
    saveTab() {
      ...
      if (this.objectType == "LOCATION") {
        ...
        axios.post("/api/tab-table/save-tab-table-mold", params).then((res) => {
          this.handleClose();
          if (isCreate) {
            this.$emit("create-success", res.data.data, {
              title: "Success!",
    ...
    async getListToolingSelectedByTab(status, isDisable = false) {
      const res = await axios.get(
        `/api/molds?filterCode=COMMON&tabId=${
          !this.tab.id ? "" : this.tab.id
        }&id=&query=&status=${
          !status ? "All" : status
        }&deleted=${isDisable}&page=1&pageType=${this.pageType}&size=9999`
      );
    ...
    async getListLocationSelectedByTab(status, isDisable = false) {
      const res = await axios.get(
        `/api/locations?tabId=${
          !this.tab.id ? "" : this.tab.id
    ...
    async getListToolingTabType(tabType) {
      const res = await axios.get(
        `/api/molds?tabId=${
          !this.tab.id ? "" : this.tab.id
        }&id=&query=&status=&tabType=${tabType}&page=1&pageType=${
          this.pageType
        }&size=9999`
      );
      this.tab.addItemIdList = res.data.content.map((i) => i.id);
    ...
    async getListLocationTabType(tabType) {
      const res = await axios.get(
        `/api/locations?tabId=${
          !this.tab.id ? "" : this.tab.id
        }&page=1&size=9999`
    ...
    async getListLocationTabDefault(status) {
      const res = await axios.get(
    ...
    async getToolingList(pageNumber) {
      this.requestParam.page = pageNumber === undefined ? 1 : pageNumber;
      const res = await axios.get(
        `/api/molds?tabId=&id=&query=${this.requestParam.query}&status=All&sort=${this.requestParam.sort}&page=${this.requestParam.page}&pageType=${this.pageType}&isModalSelected=true&filterCode=COMMON`
      );
      this.toolingPerPageList = res.data.content;
      this.toolingOptionsAdd[0].title =
        "Add Page (" +
    ...
    async showCreateTabView(type, tab) {
      ...
      if (type === "EDIT") {
        ...
        if (this.objectType == "LOCATION") {
          this.getListLocationSelectedByTab();
        } else {
          this.getListToolingSelectedByTab();
        }
    ...
    getPeriodValue() {
      ...
      const periodValue = `${currentDate.getFullYear()}${weekNumber}`;
      return periodValue;
    },
  },
};
