...
module.exports = {
  ...
  methods: {
    ...
    async saveTab() {
      ...
      try {
        if (isCreate) {
          const { data: terminalTabData } = await axios.post(
            `${API_BASE_TAB_STP}/TERMINAL/one`,
            dataBody
          );
          this.handleClose();
          this.$emit("create-success", terminalTabData, {
            title: "Success!",
            content: `Your tab view has been created`,
          });
        } else {
          const { data: terminalTabData } = await axios.put(
            `${API_BASE_TAB_STP}/TERMINAL/${tab.id}`,
            dataBody
          );
          this.$emit("edit-success", terminalTabData, {
            title: "Success!",
    ...
    async getListDataSelectedByTab(tabName) {
      ...
      const res = await axios.get(`${API_BASE_TMN_STP}?${param}`);
