  ...
  <div class="data-table__main">
    <table class="data-grid__table">
        ...
        <tr v-show="showActionBar" class="data-grid__row">
          ...
          <th class="data-grid__cell" colspan="100">
            <div class="action-bar">
              <span
                v-for="item in listActionsEnable"
                :key="item.key"
                class="action-item"
                @click="handleClickAction(item.key)"
              >
                <span class="action-item__label">{{ item.label }}</span>
                <span
                  class="action-item__icon"
                  :style="`mask-image: url(${item.iconSrc}); -webkit-mask-image: url(${item.iconSrc}); ${item.iconStyle}`"
                ></span>
              </span>
            </div>
          </th>
          ...
          >
            <span class="cell-inner">
              ...
              <span
                ...
                :class="{ desc: sortDesc && col.field === sortColumn }"
      ...
      <tbody class="data-grid__body">
        <tr
          v-for="(row, rowIndex) in listRenderedItems"
          :key="rowIndex"
        ...
        >
          <td class="data-grid__cell cell-checkbox">
            <input
              type="checkbox"
              v-model="listRenderedItems[rowIndex].checked"
            />
          </td>
          <td
            v-for="col in enabledColumns"
    ...
    <div class="data-grid__pagination">
      <ul class="pagination">
        ...
        >
          <a class="page-link" @click="$emit('paginate', data.pageNumber)">
            {{ data.text }}
          </a>
        </li>
      </ul>
...
module.exports = {
  ...
  setup(props, { root, emit, refs }) {
    ...
    const listSelectedItems = computed(() =>
      listRenderedItems.value.filter((i) => i.checked)
    );

    watch(
      () => props.listData,
      (newVal) => {
        console.log("props.listData/newVal");
        ...
        console.log("listRenderedItems", listRenderedItems);
      },
      { immediate: true }
    );

    watch(isCheckAll, (newVal) => {
      listRenderedItems.value = listRenderedItems.value.map((i) => ({
        ...i,
        checked: newVal,
      }));
    });
    watch(listSelectedItems, (newVal) => {
      if (
        listRenderedItems.value.length &&
        newVal.length === listRenderedItems.value.length
      ) {
    ...
    const allColumns = [
      {
        label: "File Group Code",
        field: "fileGroupCode",
        subField: "fileGroupName",
        sort: true,
        enabled: true,
      },
      {
    ...
    const enabledColumns = computed(() =>
      columnConfig.value.filter((c) => c.enabled)
    );
    console.log(
      "🚀 ~ file: file-manage-table.vue:230 ~ setup ~ enabledColumns:",
      enabledColumns
    );
    ...
    const listActions = reactive([
      {
        key: "view",
        label: "View",
        iconSrc: "/images/icon/eye.svg",
        iconStyle: "width: 13.77px; height: 8.52px;",
        show: computed(
          () =>
            listSelectedItems.value.length === 1 &&
            ["RELEASED"].includes(listSelectedItems.value?.[0].fileGroupStatus)
        ),
      },
      {
        key: "update",
        label: "Update",
        iconSrc: "/images/icon/edit-2.svg",
        iconStyle: "width: 11px; height: 12.57px;",
        show: computed(
          () =>
            listSelectedItems.value.length === 1 &&
            listSelectedItems.value?.[0]?.enabled &&
            ["UNRELEASED"].includes(
              listSelectedItems.value?.[0]?.fileGroupStatus
            )
        ),
      },
      {
        key: "disable",
        label: "Disable",
        iconSrc: "/images/icon/disable.svg",
        iconStyle: "width: 12.41px; height: 12.29px;",
        show: computed(
          () => listSelectedItems.value.length && props.activeTab === "enabled"
        ),
      },
      {
        key: "enable",
        label: "Enable",
        iconSrc: "/images/icon/new/enable.svg",
        iconStyle: "width: 12.41px; height: 12.29px;",
        show: computed(
          () => listSelectedItems.value.length && props.activeTab === "disabled"
        ),
      },
      {
        key: "manage",
        label: "Manage Version",
        iconSrc: "/images/icon/version.svg",
        iconStyle: "width: 13px; height: 12px;",
        show: computed(
          () =>
            listSelectedItems.value.length === 1 &&
            listSelectedItems.value?.[0]?.enabled
    ...
    const listActionsEnable = computed(() =>
      listActions.filter((i) => i.show.value)
    );
    const showActionBar = computed(() => listActions.some((i) => i.show.value));

    watchEffect(() => console.log("props.listData", props.listData));
    watchEffect(() =>
      console.log("listSelectedItems", listSelectedItems.value)
    );
