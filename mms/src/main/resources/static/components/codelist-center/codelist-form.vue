              ...
              <div class="card-body custom-body">
                ...
                <div class="form-group d-flex custom-row">
                  ...
                  >
                    <div class="position-relative">
                      ...
                      <common-button
                        :title="role.group1Title"
                        @click="handleToggle"
                        :is-show="showMainLevel"
                      >
                      </common-button>
                      <common-select-popover
                        :is-visible="showMainLevel"
                        @close="handleCloseToggle"
                        class="custom-popover"
                      >
                        <common-select-dropdown
                          :searchbox="false"
                          :class="{ show: showMainLevel }"
                          :style="{ position: 'static' }"
                          :items="MainLevelData"
                          :checkbox="false"
                          class="dropdown min-vw-0"
                ...
                <div class="form-group d-flex custom-row">
                  ...
                  >
                    <div class="position-relative">
                      <common-button
                        :title="role.group2Title"
                        @click="handleToggle2"
                        :is-show="showSubLevel"
                      >
                      </common-button>
                      <common-select-popover
                        :is-visible="showSubLevel"
                        @close="handleCloseToggle2"
                        class="custom-popover"
                      >
                        <common-select-dropdown
                          :searchbox="false"
                          :class="{ show: showSubLevel }"
                          :style="{ position: 'static' }"
                          :items="SubLevelData"
                          :checkbox="false"
                          class="dropdown min-vw-0"
                          :click-handler="select2"
                          :set-result="() => {}"
                        ></common-select-dropdown>
                      </common-select-popover>
                    </div>
                ...
                <div class="flex-md-row-reverse btn-submission">
                  <button
                    ...
                    :class="
                      validateRole ? 'disable-button-edit' : 'button-edit'
                    "
                  >
                    Save
                  </button>
...
module.exports = {
  ...
  data() {
    return {
      codeName: "Machine Downtime Reason",
      compName: "modal",
      intialSelect: true,
      intialSelect2: true,
      SubLevelData: [],
      ...
      role: {
        ...
        companyCode: "",
        companyId: "",
        codeType: "MACHINE_DOWNTIME_REASON",
        code: "",
        title: "",
  ...
  methods: {
    callbackCompany: function (cbObject) {
      ...
      // this.role.companyId로 dropdown List 재생성

      axios
        .get(
          `/api/common/cod-stp/drop-down/MACHINE_DOWNTIME_REASON_GROUP1?companyId=${this.role.companyId}`
        )
        .then((res) => {
          this.MainLevelData = res.data;
          console.log("MainLevelData: ", this.MainLevelData);
        });
      axios
        .get(
          `/api/common/cod-stp/drop-down/MACHINE_DOWNTIME_REASON_GROUP2?companyId=${this.role.companyId}`
        )
        .then((res) => {
          this.SubLevelData = res.data;
          console.log("SubLevelData: ", this.SubLevelData);
        });
    },

    select(item) {
      this.role.group1Code = item.code;
      this.role.group1Title = item.title;
      this.showMainLevel = false;
    },

    handleToggle: function (isShow) {
      this.showMainLevel = isShow;
    },

    handleCloseToggle: function (isShow) {
      this.showMainLevel = false;
    },

    select2(item) {
      this.role.group2Code = item.code;
      this.role.group2Title = item.title;
      this.showSubLevel = false;
    },

    handleToggle2: function (isShow) {
      this.showSubLevel = isShow;
    },

    handleCloseToggle2: function (isShow) {
      this.showSubLevel = false;
    },
    ...
    async submit() {
      if (!this.role.group1Code) {
        return Common.alert("Main Level을 선택해주세요");
      }
      if (!this.role.group2Code) {
        this.role.group2Title = "";
        // return Common.alert('Sub Level을 선택해주세요');
      }
    ...
    async create() {
      await axios
        ...
        .catch((error) => {
          if (error.response) {
            Common.alert(error.response.data.message);
          }
        });
    },
