      ...
      >
          ...
          <div class="container-date-pick">
            ...
            >
              ...
              <div class="block-footer">
                <span
                  >*Click on the number in the calendar to choose the date.</span
                >
              </div>
            </div>
              ...
              >
                <div v-for="i in getNumberOfWeek()" :key="i" class="week-item">
                  ...
                  <span
                    ...
                    :class="{
                      'selected-week':
                        (i == selectedItem.selectedWeek || i == selectedItemSecond.selectedWeek) &&
                        (currentYearForWeek == selectedItem.selectedYear || currentYearForWeek == selectedItemSecond.selectedYear),
                      selected:
                        (i == selectedItem.selectedWeek || i == selectedItemSecond.selectedWeek) &&
                        (currentYearForWeek == selectedItem.selectedYear || currentYearForWeek == selectedItemSecond.selectedYear),
                    }"
            ...
            <div class="block-button">
                  ...
                  <a-tooltip placement="bottomLeft">
                    <template slot="title">
                      <div style="font-size: 11px; color: #ebebeb">
                        Time range is fixed for Weekly option. 
                      </div>
                    </template>
                    <input
                      type="text"
              ...
              <div v-else-if="selectedType == 'MONTHLY'" class="d-flex">
                <div>
                  ...
                  <a-tooltip placement="bottomLeft">
                    <template slot="title">
                      <div style="font-size: 11px; color: #ebebeb">
                        Time range is fixed for Monthly option.
                      </div>
                    </template>
                    <input
                      type="text"
              ...
              <div v-else-if="selectedType == 'YEARLY'" class="d-flex">
                <div :key="textboxKey">
                  ...
                  <a-tooltip placement="bottomLeft">
                    <template slot="title">
                      <div style="font-size: 11px; color: #ebebeb">
                        Time range is fixed for Yearly option. 
                      </div>
                    </template>
                    <input
                      type="text"
              ...
              <div v-else-if="selectedType == 'DAILY'" class="d-flex">
                <div>
                  <span class="right-title">From</span><br />
                  <input
                    type="text"
        ...
        <div style="display: flex; justify-content: flex-end; margin-top: 9px">
          <a
            ...
            id="light-animation"
            class="
              btn-custom btn-custom-date btn-custom-date-light
              animationPrimary
              animationOutline
            "
          >
...
module.exports = {
  ...
  data() {
    return {
      ...
      selectedItem: {
        selectedMonth: null,
        selectedMonthSecond: null,
        selectedYear: null,
        selectedWeek: null,
        selectedWeekSecond: null,
      },
      selectedItemSecond: {
        selectedMonth: null,
        selectedYear: null,
  ...
  computed: {
    checkEnable() {
      ...
      switch (this.selectedType) {
        ...
        case "WEEKLY":
          if (
            ...
            this.selectedItemSecond.selectedYear != null
          ) {
            result = true;
          }
          break;
      ...
      return this.dataFrequency && this.dataType && this.dataType.length > 0 && result;
      // return (
      //   this.dataFrequency &&
      //   this.dataType &&
  ...
  watch: {
    ...
    singleDate: function(newValue){
      if (newValue != null) {
        this.dateFrom = `${newValue.getFullYear()}-${
              newValue.getMonth() < 9
  ...
  methods: {
    datePickerMove: function(event) {
      ...
      let y = event.clientY;
      console.log('x:',x,'y:',y);
      let arrRangeTemp=document.querySelectorAll('.vc-pane-container .vc-day span[style*="color: var(--blue-900);"]');
    ...
    getDateOfISOWeek(w, y) {
      let simple = null;
      if (moment("01-01-" + y,"MM-DD-YYYY").toDate().getDay() < 4) {
    ...
    getNumberOfWeek() {
      ...
      if (moment("01-01-" + this.currentYearForWeek,"MM-DD-YYYY").toDate().getDay() >= 4) {
        result--;
      }
      if (moment("12-31-" + this.currentYearForWeek,"MM-DD-YYYY").toDate().getDay() < 4) {
        result--;
      }
      return result;
    },
    changeSelectYear(year,type){
      if (type == 'week') {
        this.currentYearForWeek = year
      } else if( type == 'month'){
        this.currentYearForMonth = year
      }
    },
    clearPickingData(){
      this.selectedItem = {
    ...
    handleSelectWeek(weekNumber, currentYearForWeek) {
      ...
      if (this.pickStep == 2) {
        ...
        if (weekNumber != this.selectedItem.selectedWeek) {
          this.selectedItem.selectedYear = currentYearForWeek
          this.selectedItem.selectedWeek = weekNumber,
          console.log(
        ...
        if (weekNumber != this.selectedItem.selectedWeek) {
          this.selectedItemSecond.selectedYear = currentYearForWeek
          this.selectedItemSecond.selectedWeek = weekNumber,
          console.log(
            `Selected Week Change to ${this.selectedItem.selectedWeek}`
          );
          this.pickStep++
      ...
      switch (this.selectedType) {
        case "WEEKLY":
          if (this.selectedItem.selectedWeek!=null) {
            ...
            var firstDay2 = new Date(
              this.selectedItemSecond.selectedYear,
              0,
              1
            ).getDay();
            var d1 = new Date(
            ...
            var d2 = new Date(
              "Jan 01, " + this.selectedItemSecond.selectedYear + " 00:00:00"
            );
            var lastWeekDate2 =
              d2.getTime() -
              3600000 * 24 * (firstDay2 - 1) +
              604800000 * this.getNumberOfWeek();
              
            var w2 =
              d2.getTime() -
              3600000 * 24 * (firstDay2 - 1) +
              604800000 * (this.selectedItemSecond.selectedWeek - 1);

            if (this.selectedItem.selectedYear == this.getNumberOfWeek()) {
              if (new Date(lastWeekDate1).getDay() < 4) {
                w1 =
                  d1.getTime() -
                  3600000 * 24 * (firstDay1 - 1) +
                  604800000 * (this.selectedItem.selectedWeek - 1);
            ...
            if (this.selectedItemSecond.selectedYear == this.getNumberOfWeek()) {
              if (new Date(lastWeekDate2).getDay() < 4) {
                w2 =
                  d2.getTime() -
                  3600000 * 24 * (firstDay2 - 1) +
                  604800000 * (this.selectedItemSecond.selectedWeek - 1);
              ...
              if (d2.getDay() > 3) {
                ...
                w2 =
                  d2.getTime() -
                  3600000 * 24 * (firstDay2 - 1) +
                  604800000 * this.selectedItemSecond.selectedWeek;
              }
            }

            n1 = new Date(w1 - 86400000);
            n2 = new Date(w2 + 432000000);
          }
          break;
        case "MONTHLY":
        ...
        if (inputDate == "") {
          ...
          if (dateParts.length == 3 && checkDate != "Invalid Date") {
            if (
              ...
              this.range.startMessage = "Please enter the date in the format ‘yyyy-mm-dd’.";
            }
          } else {
        ...
        if (inputDate == "") {
          ...
          if (dateParts.length == 3 && checkDate != "Invalid Date") {
            if (
              ...
              this.range.endMessage = "Please enter the date in the format ‘yyyy-mm-dd’.";
              console.log(
    ...
    handleShowModal(data){
      console.log(data)
      this.selectedType = data.rangeType
      this.dateFrom = data.from
      this.dataTo = data.to
      switch (data.rangeType) {
        case 'DAILY':
          this.singleDate = data.time.from
          break;
        case 'WEEKLY':
          this.singleDate = data.time.from
          this.selectedItem = {
            selectedMonth: null,
            selectedYear: data.time.from.getFullYear(),
            selectedWeek: this.getCurrentWeekNumber(data.time.from),
          }
          this.currentYearForWeek = data.time.from.getFullYear()
          break;
        case 'MONTHLY':
          this.singleDate = data.time.from
          this.selectedItem = {
            selectedMonth: data.time.from.getMonth(),
            selectedYear: data.time.from.getFullYear(),
            selectedWeek: null,
          }
          this.currentYearForMonth = data.time.from.getFullYear()
          break;
      }
    ...
    closeDatePicker() {
                    const root = document.getElementById('date-picker-modal')
                    if (root) {
