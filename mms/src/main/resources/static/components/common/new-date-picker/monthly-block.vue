...
module.exports = {
  ...
  watch: {
    ...
    frequency(newValue) {
       if (newValue.includes("MONTH")) {
        console.log('%c MONTH is selected', 'background : #FF8C2C, color : #ffffff')
  ...
  methods: {
    setDefault(date) {
      if (date) {
        ...
        if (this.isRange) {
          this.selectedMonth = +monthFrom;
          this.selectedSecondMonth = +monthTo;
    ...
    clearPickingData() {
      ...
      this.pickStep = 0;
      this.handleSelectMonth(this.selectedMonth, this.selectedYear)
    },
