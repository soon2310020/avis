...
module.exports = {
  ...
  created() {
    ...
    this.selectedWeek = this.getCurrentWeekNumber(this.today);
  },
  mounted() {
  ...
  computed: {
    getCurrentWeek() {
      // let weekNum = moment(this.today).format("w");
  ...
  watch: {
    ...
    frequency(newValue) {
      if (newValue.includes("WEEK")) {
        console.log('%c WEEK is selected', 'background : #FF8C2C; color : #FFFFFF')
  ...
  methods: {
    setDefault(date) {
      if (date) {
        console.log('hurraah i am run yayyyyy!!!!')
    ...
    getCurrentYear(currentDate) {
      let year = currentDate.from.getFullYear()
      let oneJan = new Date(currentDate.from.getFullYear(), 0, 1);
      let numberOfDays = Math.floor((currentDate.from - oneJan) / (24 * 60 * 60 * 1000));
    ...
    getTitle() {
      ...
      if (this.isRange) {
        if (this.selectedWeek) {
          start = `${this.selectedYear}-${this.selectedWeek > 9 ? this.selectedWeek : "0" + this.selectedWeek
            }`;
        }
        if (this.selectedSecondWeek) {
          end = `${this.selectedSecondYear}-${this.selectedSecondWeek > 9
              ? this.selectedSecondWeek
        ...
        if (this.selectedWeek) {
          ...
          end = `${this.selectedYear}-${this.selectedWeek > 9 ? this.selectedWeek : "0" + this.selectedWeek
            }`;
        }
      }
      return {
    ...
    clearPickingData() {
      ...
      this.handleSelectWeek(this.selectedWeek,this.selectedYear );
    },
