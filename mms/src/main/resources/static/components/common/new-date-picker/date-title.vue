<template>
  <div>
    <div v-if="enableEdit" style="gap : 25px" class="d-flex justify-content-start">
      <div>
    ...
    <div v-else style="gap : 25px" class="d-flex justify-content-start">
      <div>
        <span class="right-title">From</span><br />
  ...
  methods: {
    handleDateFromInput: _.debounce(function (event) {
      ...
      if (this.range.startMessage == 'valid' && this.range.endMessage == 'valid') {
        this.$emit('title-change', true, {start: this.range.startMessage, end: this.range.endMessage})
      } else {
