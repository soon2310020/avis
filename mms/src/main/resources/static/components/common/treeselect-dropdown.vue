...
<template>
  <div class="dropdown-wrap" :style="wrapStyle">
    ...
    <div class="buttons-custom w-100 align-items-center d-flex justify-content-between">
...
module.exports = {
  ...
  props: {
    ...
    wrapStyle: Object, // Set whether dropdown-list wrap-style or not (ex: {top: '10px', left: '10px', width: '10px'})
  ...
  methods: {
      ...
      this.$nextTick(() => {
        ...
        if (self.applyHandler) {
         ...
         self.applyHandler(self.checkedArray);
          // self.clickHandler(self.checkedArray);
        }
      });
      this.$forceUpdate();
    ...
    getTitle(text){
      if (!this.isMoldList && this.id === 'mold-filter') {
        if (text === 'Installed') {
          return 'Matched'
        }
      }
