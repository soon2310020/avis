...
module.exports = {
  ...
  data() {
    return {
      modalTitle: "Cycle Time Deviation Details",
      listColumns: [
        {
          label: "Toolings",
          field: "moldCode",
        ...
        },
        {
          ...
          textAlign: "right",
          tooltip:
            "Normalized CT Deviation is represented as an index. The lower the value, the higher the score in terms of optimal, consistent production.",
          formatter: (val) => {
            if (val == null) return "-";
  ...
  methods: {
    ...
    handleChangePage(type) {
      console.log("handleChangePage", this.isLoading);
      if (!this.isLoading) {
        if (type === "NEXT") {
        ...
        this.fetchData({ page: this.pagination.page, sort: this.sortField });
      }
    },
  ...
  watch: {
    ...
    //   this.fetchData({ page: newVal, sort: this.sortField });
    // },
    sortField(newVal) {
      this.fetchData({ sort: newVal });
    },
    visible(newVal) {
      if (!newVal) {
