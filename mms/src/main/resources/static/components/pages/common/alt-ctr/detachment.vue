<template>
  <div v-if="isSubmenuPermitted">
    <notify-alert type="DETACHMENT" @change-alert-generation="changeAlertGeneration" :resources="resources"></notify-alert>
    <div class="row">
      <div class="col-lg-12">
          ...
          <div class="card-body">
            ...
            <div class="form-row">
              <div class="mb-3 mb-md-0 col-12 ">
                <common-searchbar :request-param="requestParam" :placeholder="resources['search_by_columns_shown']" :on-search="search"></common-searchbar>
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="card wave1">
          ...
          <div class="card-body">
            <ul class="nav nav-tabs" style="margin-bottom: -1px">
              <li class="nav-item">
                <a class="nav-link" :class="{ active: requestParam.status == 'alert' }" href="#" @click.prevent="tab('alert')"><span v-text="resources['alert']"></span>
              ...
              <table class="table table-responsive-sm table-striped">
                <thead id="thead-actionbar" class="custom-header-table">
                  <tr :class="{ 'invisible': listChecked.length != 0 }" style="height: 57px" class="tr-sort">
                    <th v-if="requestParam.status !== 'history'" style="width: 40px; vertical-align: middle!important; text-align: center;">
                    ...
                    <template v-for="(column, index) in allColumnList" v-if="column.enabled && !column.hiddenInToggle">
                      <th v-if="column.field === 'confirmedAt' && requestParam.status === 'history'" :class="[{ active: currentSortType === 'confirmedAt' }, isDesc ? 'desc' : 'asc', 'text-left', '__sort']">
                        <span v-text="resources['confirmation_date']"></span>
                        <span @click="sortBy('confirmedAt')" class="__icon-sort"></span>
                      </th>
                      <th :class="[{ active: currentSortType === 'moldLocationStatus' }, isDesc ? 'desc' : 'asc', 'text-center', '__sort']" v-else-if="column.field === 'moldLocationStatus' && requestParam.status === 'alert'">
                        <span v-text="resources['status']"></span>
                        <span @click="sortBy('moldLocationStatus')" class="__icon-sort"></span>
                      </th>
                      <th :key="index" :class="[{ __sort: column.sortable, active: currentSortType === column.sortField, }, isDesc ? 'desc' : 'asc', column.label === 'OP' || column.label === 'Status' ? '' : 'text-left',]" @click="sortBy(column.sortField)"
                        v-if="column.field !== 'confirmedAt' && column.field !== 'moldLocationStatus'">
                        <span>{{ column.label }}</span>
                        <span v-if="column.sortable" class="__icon-sort"></span>
                      </th>
                    </template>
                    <th class="text-center" v-if="requestParam.status === 'history'"><span v-text="resources['action']"></span></th>
                  </tr>
                ...
                <tbody class="op-list" style="display:none">
                  <tr v-show="!isLoading" v-for="(result, index, id) in results" :id="result.id">
                    ...
                    <template v-for="(column, columnIndex) in allColumnList" v-if="column.enabled && !column.hiddenInToggle">
                      ...
                      <td class="text-left" v-else-if="column.field === 'location'">
                        <div class="font-size-14">
                          <a href="#" @click.prevent="showLocationHistory(result)"> {{ result.mold.locationName }} </a>
                        </div>
                        <span class="small text-muted font-size-11-2"> {{ result.mold.locationCode }} </span>
                      </td>
                      <!-- Detachment Date -->
                      <td class="text-left" v-else-if="column.field === 'detachmentTime'"> {{ convertDate(formatToDateTime(result.detachmentTime)) }}
                        <!-- {{formatToDateTime(result.detachmentTime)}} -->
                        <div>
                          <span class='font-size-11-2'> {{ convertTimeString(formatToDateTime(result.detachmentTime)) }}
                            <!-- {{formatToDateTime(result.detachmentTime)}} -->
                          </span>
                        </div>
                      </td>
                      <!-- Confirmation Date -->
                      <td class="text-left" v-else-if="column.field === 'confirmedAt' && requestParam.status === 'history'">
                        <div>
                          <span class='font-size-14'> {{ convertTime(result.confirmedAt) }}
                            <!-- {{result.confirmedAt}} -->
                          </span>
                        </div>
                      </td>
                      <!-- OP -->
                      <td style="vertical-align: middle!important;" class="text-center op-status-td" v-else-if="column.field === 'operatingStatus'">
                      ...
                      <!--status -->
                      <td style="vertical-align: middle!important;" class="text-center op-status-td" v-else-if="requestParam.status === 'alert' && column.field === 'moldLocationStatus'">
                        <span class="label label-danger" v-if="result.equipmentStatus === 'DETACHED'">Detached</span>
                      </td>
                    ...
                    <td style="vertical-align: middle!important;" class="text-center" v-if="requestParam.status === 'history'">
                      <div class="d-flex align-items-center justify-content-center">
                        <span v-if="requestParam.status === 'history'" style="cursor: pointer;" v-on:click="showRelocationConfirmHistory(result.mold)"><img height="18" src="/images/icon/view.png" alt="View" /></span>
    ...
    <detachment-confirm-history :resources="resources"></detachment-confirm-history>
  </div>
</template>
...
module.exports = {
  ...
  components: {
    'detachment-confirm-history': httpVueLoader('/components/detachment-confirm-history.vue'),
  ...
  methods: {
    ...
    setAllColumnList() {
      this.allColumnList = [
        ...
        { label: this.resources['location'], field: 'location', default: true, sortable: true, sortField: 'mold.location.name', defaultSelected: true, defaultPosition: 2 },
        { label: this.resources['detachment_time'], field: 'detachmentTime', default: true, sortable: true, sortField: 'detachmentTime', defaultSelected: true, defaultPosition: 3 },
        { label: this.resources['confirmation_date'], field: 'confirmedAt', default: true, sortable: true, sortField: 'confirmedAt', defaultSelected: true, defaultPosition: 4 },
    ...
    showRelocationConfirmHistory: function (mold) {
      var child = Common.vue.getChild(this.$children, 'detachment-confirm-history');
      if (child != null) {
    ...
    paging: function (pageNumber) {
      ...
      axios.get('/api/molds' + '/detachment?lastAlert=true&' + param, {
