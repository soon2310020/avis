            ...
            <div class="content-group-wrapper">
              ...
              >
                ...
                <div class="col-md-3">
                  ...
                  <div v-if="data.projectName && data.projectName.length > 17">
                    <a-tooltip placement="bottom">
                      <template slot="title">
                        <div style="padding: 6px; font-size: 13px">
                          {{ data.projectName }}
                      ...
                      <div class="small text-muted font-size-11-2">
                        {{ truncateText(data.projectName, 17) }}
                      </div>
                    </a-tooltip>
                  </div>
...
module.exports = {
  ...
  methods: {
    ...
    saveTab() {
      ...
      axios.post("/api/tab-table/save-tab-table-part", params).then((res) => {
        this.handleClose();
        if (isCreate) {
    ...
    async getListDataSelectedByTab(isDisable = false) {
    ...
    async getAllDataListSearching(pageNumber) {
      ...
      const res = await axios.get(
        `/api/parts?tabId=&id=&query=${this.requestParam.query}&status=active&sort=${this.sortData}&page=${this.requestParam.page}&pageType=${this.pageType}&isModalSelected=true`
      );
    ...
    async showCreateTabView(type, tab) {
      ...
      if (type === "EDIT") {
        ...
        this.getListDataSelectedByTab(tab.name === "Disabled");
      } else {
