        ...
        <div class="modal-body" style="padding-left: 40px; padding-right: 40px">
          ...
          <div class="container content-group">
            <div class="content-group-header">
              <div class="row">
                <div class="col-md-4 tab-group-title active">
                  <p v-text="resources['company_name']"></p>
                  <span
                    class="__icon-sort"
                    :class="[
                      sortField === 'partCode' && sortType === 'desc'
                        ? 'desc'
                        : 'asc',
                    ]"
                    @click="sort('name')"
                ...
                <div class="col-md-4 tab-group-title">
                  <p v-text="resources['company_code']"></p>
                  <span
                    class="__icon-sort"
                    :class="[
                    ...
                    @click="sort('companyCode')"
                  ></span>
                </div>
                <div class="col-md-4 tab-group-title"></div>
              </div>
            </div>
            <div class="content-group-wrapper">
              ...
              >
                    ...
                    <div>
                      <div v-if="data.name && data.name.length > 17">
                        <a-tooltip placement="bottom">
                          ...
                          <a href="#" @click.prevent="showCompanyDetails(data)">
                ...
                <div class="col-md-4">
                  <div class="tab-group-item">
                    <div
                      v-if="data.companyCode && data.companyCode.length > 17"
                    >
                      <a-tooltip placement="bottom">
                        <template slot="title">
                          <div style="padding: 6px; font-size: 13px">
                        ...
                        <span>
                          {{ truncateText(data.companyCode, 17) }}
                        </span>
                      </a-tooltip>
                    </div>
                    <span v-else>
...
module.exports = {
  ...
  methods: {
    ...
    async saveTab() {
      ...
      // axios
      //   .post("/api/tab-table/save-tab-table-company", params)
      //   .then((res) => {
      //     this.handleClose();
      //     if (isCreate) {
      ...
      try {
        if (isCreate) {
          const { data: companyTabData } = await axios.post(
            `${API_BASE_TAB_STP}/COMPANY/one`,
            dataBody
          );
          this.handleClose();
          this.$emit("create-success", companyTabData, {
            title: "Success!",
            content: `Your tab view has been created`,
          });
        } else {
          const { data: companyTabData } = await axios.put(
            `${API_BASE_TAB_STP}/COMPANY/${tab.id}`,
            dataBody
          );
          this.$emit("edit-success", companyTabData, {
            title: "Success!",
            content: `Your tab view has been edited`,
          });
        }
      } catch (error) {
        console.log("tab-view-company-modal:::saveTab:::error", error);
      }
    },
    validForm() {
      if (!this.tab.name) {
    ...
    async getListDataSelectedByTab(tabName) {
      ...
      //   }&id=&query=&status=${this.getStatus(tab)}&page=1&size=9999`
      // );
      ...
      const res = await axios.get(`${API_BASE_COM_STP}?${param}`);
    ...
    async getAllDataList() {
      // // OLD API
      // const res = await axios.get(
      //   `/api/companies?tabId=&id=&query=${
      //     this.requestParam.query || ""
      //   }&status=active&sort=${
      //     this.sortData
      //   }&page=1&isModalSelected=true&size=99999`
      // );
    ...
    async getAllDataListSearching(pageNumber) {
      ...
      // const res = await axios.get(
      //   `/api/companies?tabId=&id=&query=${this.requestParam.query}&status=active&sort=${this.sortData}&page=${this.requestParam.page}&isModalSelected=true`
      // );
