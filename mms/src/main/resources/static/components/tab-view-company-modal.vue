...
module.exports = {
  ...
  methods: {
    ...
    async getListDataSelectedByTab(tab, isDisable = false) {
    ...
    async getAllDataList() {
      const res = await axios.get(
        `/api/companies?tabId=&id=&query=&status=active&sort=${this.sortData}&page=1&isModalSelected=true&size=99999`
      );
    ...
    showCreateTabView(type, tab) {
      ...
      if (type === "EDIT") {
        ...
        this.getListDataSelectedByTab(tab, tab.name === "Disabled");
      } else {
