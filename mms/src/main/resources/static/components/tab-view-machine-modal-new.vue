...
module.exports = {
  ...
  methods: {
    ...
    async saveTab() {
      ...
      try {
        if (isCreate) {
          const { data: machineTabData } = await axios.post(
            `${API_BASE_TAB_STP}/MACHINE/one`,
            dataBody
          );
          this.handleClose();
          this.$emit("create-success", machineTabData, {
            title: "Success!",
            content: `Your tab view has been created`,
          });
        } else {
          const { data: machineTabData } = await axios.put(
            `${API_BASE_TAB_STP}/MACHINE/${tab.id}`,
            dataBody
          );
          this.$emit("edit-success", machineTabData, {
            title: "Success!",
            content: `Your tab view has been edited`,
          });
        }
      } catch (error) {
        console.log("tab-view-terminal-modal:::saveTab:::error", error);
      }
    },
    validForm() {
      if (!this.tab.name) {
    ...
    async getListDataSelectedByTab(tabName) {
      ...
      const res = await axios.get(`${API_BASE_MCH_STP}?${param}`);
