                      ...
                      <div class="action-bar">
                        <div
                          v-if="
                            ...
                            listCheckedFull[0].sensorStatus ===
                              'NOT_INSTALLED' &&
                            requestParam.tabName !== 'Disabled'
                          "
                          class="action-item"
                          @click="installCounter(listCheckedFull[0].id)"
                        >
                          <span>Match</span>
                ...
                <tr v-for="(result, index, id) in results" :id="result.id">
                  ...
                  >
                    ...
                    >
                      <div v-if="result.locationCode">
                        <div
                          class="font-size-14"
                          v-html="
                            locationName(result.locationName, result.enabled)
                          "
                        ></div>
                        <div class="small text-muted font-size-11-2">
                          {{ result.locationCode }}
                    ...
                    >
                      ...
                      ></span>
                      <span
                        v-if="result.operatingStatus == 'INACTIVE'"
                    ...
                    >
                      <span
                        v-if="
                          result.presetStatus == 'READY' ||
                          result.presetStatus == 'IN_PROGRESS'
                        "
                    ...
                    >
                      <a-tooltip placement="bottomLeft">
                        ...
                        <span
                          v-if="result.toolingStatus === 'IN_PRODUCTION'"
                        ...
                        ></span>
                        <span
                          v-if="result.toolingStatus === 'INACTIVE'"
                          class="op-active label label-not-working"
                        ></span>
                        <span
                          v-if="result.toolingStatus === 'SENSOR_OFFLINE'"
                      ...
                      <a-tooltip placement="bottomLeft">
                        <template slot="title">
                          <div
                            class="tooltip-label"
                            v-text="
                              getTooltipCounterStatus(result.sensorStatus)
                            "
                          ></div>
                        </template>
                        <div
                          v-if="result.sensorStatus === 'INSTALLED'"
                          class="counter-status-label counter-installed-label"
                        >
                          Installed
                        </div>
                        <div
                          v-if="result.sensorStatus === 'NOT_INSTALLED'"
                          class="counter-status-label counter-not-installed-label"
                        >
                          Not Installed
                        </div>
                        <div
                          v-if="result.sensorStatus === 'DETACHED'"
...
const API_BASE_SSR_STP = Common.getApiBase().COUNTER;
...
module.exports = {
  components: {
    ...
    "tab-view-modal": httpVueLoader(
      "/components/tab-view-counter-modal-new.vue"
    ),
  ...
  methods: {
    handleCreateNewSensor() {
      Common.onChangeHref("/admin/counters/new");
    },
    async handleSubmitNote(data) {
      ...
      try {
        await axios.post(`${API_BASE_SSR_STP}/note-batch?${param}`, data);
      } catch (error) {
        console.log(error);
      }
    },
    ...
    async removeFromTab() {
      try {
        ...
        await axios.delete(`${API_BASE_SSR_STP}/tab-items-batch?${param}`);
        this.moveSuccess({
          title: "Success!",
          content: `Your data has been removed from ${this.currentActiveTab.name} tab`,
        });
      } catch (error) {
        console.log("devices-sensor-new:::removeFromTab:::error", error);
      }
    },
    moveSuccess(toast) {
    ...
    async showTab(tab) {
      try {
        ...
        if (tab.defaultTab) {
          await axios.put(
            `${API_BASE_TAB_STP}/COUNTER/names/${tab.name}/shown`
          );
        } else {
          await axios.put(`${API_BASE_TAB_STP}/COUNTER/${tab.id}/shown`);
        }
      } catch (error) {
        console.log("devices-sensor-new:::showTab:::error", error);
      }
    },
    async closeTab(tab) {
      try {
        ...
        if (tab.defaultTab) {
          await axios.put(
            `${API_BASE_TAB_STP}/COUNTER/names/${tab.name}/hidden`
          );
        } else {
          await axios.put(`${API_BASE_TAB_STP}/COUNTER/${tab.id}/hidden`);
        }
      } catch (error) {
        console.log("devices-sensor-new:::closeTab:::error", error);
      }
    },
    closeToastAlert() {
    ...
    async moveToTab(tab) {
      ...
      try {
        await axios.put(`${API_BASE_SSR_STP}/move-tab-items-batch?${param}`);
        this.moveSuccess({
          title: "Success!",
    ...
    async getTabsByCurrentUser(isCallAtMounted) {
      try {
        const { data } = await axios.get(`${API_BASE_TAB_STP}/COUNTER`);
    ...
    getOpConfig() {
      Common.getCurrentOpConfig()
        .then((data) => {
          ...
          console.log("getOpConfig 2", this.ruleOpStatus);
        })
        .catch((error) => console.log("getOpConfig error", error));
    },
      ...
      this.ListFilterItems = [
        {
          ...
          childrens: [
            ...
            { title: "Inactive", code: "INACTIVE", checked: true },
            ...
            { title: "Disabled", code: "DISABLED", checked: true },
            { title: "Unknown", code: "UNKNOWN", checked: true },
          ],
        },
        {
          id: "status",
    ...
    handlerForFilter(items) {
      ...
      this.requestParam.toolingStatus = _moldStatusList;
      this.requestParam.counterStatus = _counterStatusList;
    ...
    saveBatch(action) {
      ...
      axios
        .put(`${API_BASE_SSR_STP}/${action}-batch?${param}`)
    ...
    paging: async function (pageNumber) {
      ...
      const payload = {
        filterCode: this.requestParam.filterCode,
        query: this.requestParam.query,
        tabName: this.requestParam.tabName,
        page: pageNumber,
        size: this.requestParam.size,
        sort: this.requestParam.sort,
        counterStatus: this.requestParam.counterStatus,
        toolingStatus: this.requestParam.toolingStatus,
      };
      ...
      try {
        const { data: sensorData } = await axios.get(
          API_BASE_SSR_STP + "?" + param,
          {
            cancelToken: this.cancelToken.token,
          }
        );
        this.total = sensorData.totalElements;
        this.results = sensorData.content;
        this.pagination = Common.getPagingData(sensorData);
        ...
        this.listSelectedTabs = this.listSelectedTabs.map((tab) => {
          const total =
            sensorData.tabs.find((sensorTab) => sensorTab.tabName === tab.name)
              ?.totalElements || 0;
          return {
            ...tab,
            total,
        ...
        if (objectId != "") {
          ...
          window.history.replaceState(null, null, "/admin/counters");
        }
        ...
        console.log("device-sensor:::paging", error);
      }
    },
    ...
    async subscriptionTermBatch(value) {
      const { filterCode, query, tabName, counterStatus, toolingStatus } =
        this.requestParam;
      let listId = [];
      if (this.selectType === "all") {
      ...
      try {
        await axios.put(
          `${API_BASE_SSR_STP}/subscription-term-batch?${param}`,
          {
            subscriptionTerm: value,
          }
        );
        this.reloadPage();
        const numberOfSensor =
          this.selectType === "all"
            ? this.listSelectedTabs.find(
                (tab) => tab.name === this.currentActiveTab.name
              ).total - listId.length
            : listId.length;
        this.showToastAlert({
          title: "Success!",
          content: `${numberOfSensor} sensor${
            numberOfSensor > 1 ? "s'" : "'s"
        ...
        console.log("devices-sensor:::subscriptionTermBatch:::error", error);
      } finally {
        this.listChecked = [];
        this.showDropdownTerm = false;
      }
