<template>
  <a-popover v-model="visible" placement="bottomLeft" trigger="click">
    <a
      @click="animationOutline()"
      :class="{ 'animation-secondary': showAnimation }"
      href="javascript:void(0)"
      id="dropdown-dashboard"
      class="btn-custom btn-outline-custom-primary dropdown-dashboard-filter"
    >
    ...
    >
      ...
      <div class="all-select-list">
        <div v-if="optionArray?.length >= 5" @click="checkedAll()">
          Select All
        </div>
        <div v-if="optionArray?.length >= 5" @click="unselectAll()">
          Unselect All
        </div>
      </div>
      <a-col
        ...
        :key="item.index"
      >
        <p-check :checked="checked(item)" @change="onChange($event, item)">
          {{ item.name }}
        </p-check>
      </a-col>
    </div>
...
module.exports = {
  ...
  methods: {
    ...
    checked: function (item) {
      ...
      console.log("checked", findIndex !== -1);
      return findIndex !== -1;
    },
    checkedAll() {
    ...
    onMouseleave() {
      if (
        this.checkedList.map((value) => value.id).toString() ===
        this.oldCheckedList.map((value) => value.id).toString()
      )
        return;
  ...
  watch: {
    optionArray: function (newValue, oldValue) {
      if (Array.isArray(this.selectedData)) {
        const data = newValue.filter((o) => this.selectedData.includes(o.id));
    ...
    selectedData(newVal) {
      if (Array.isArray(newVal)) {
        const data = this.optionArray.filter((o) => newVal.includes(o.id));
        this.checkedList = data;
        this.oldCheckedList = data;
  ...
  mounted() {
    ...
    //   oldCheckedList: this.oldCheckedList,
    // });
    this.onMouseleave();
  },
};
