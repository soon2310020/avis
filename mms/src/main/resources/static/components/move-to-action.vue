  ...
  <a-popover v-model="visibleMove" placement="bottomLeft" trigger="click">
    ...
    <div slot="content">
      <ul class="child-item move-to-action">
        ...
        <li
          v-if="!subItem.isDefaultTab && subItem.name !== currentActiveTab.name"
          v-for="(subItem, index2) in tabFiltered"
          :key="`${index2}`"
          class="child-item-li"
        >
          <label @click="addToTab(subItem)">
...
module.exports = {
  ...
  methods: {
    ...
    removeFromTab() {
      ...
      axios.post(
        `/api/tab-table/save-tab-table-${this.typeView || "mold"}`,
        params
      );
    },
    addToTab(tab) {
      ...
      const params = {
        ...
        isDuplicate: false,
        removeItemIdList: [],
        addItemIdList: this.listChecked,
      };
