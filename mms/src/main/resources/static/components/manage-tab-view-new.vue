        ...
        >
          ...
          <div class="container content-group">
            <div class="row">
              ...
              <div class="col-md-12 tab-group-title">
          ...
          <div class="modal-body">
            <div class="modal-body-content">
              ...
              <div class="duplicated-title" style="font-size: 19px">
                <p>
                  Tooling data from deleted tab can still be accessible via eMoldino
                  standard tabs.
                </p>
...
module.exports = {
  ...
  watch: {
    ...
    searchedTabList: {
      handler: function () {
        let __standardTabList = this.searchedTabList.filter((tab) => tab.defaultTab);
        let __createByMeTabList = this.searchedTabList.filter((tab) => !tab.defaultTab);
        const maxLength = __createByMeTabList.length > 4 ? __createByMeTabList.length : 4;
  ...
  methods: {
    ...
    show: async function () {
      ...
      // NEW API
      try {
        const { data: dataTab } = await axios.get(
          `${API_BASE_TAB_STP}/${this.objectType}`
        );
        this.allTabList = dataTab?.content;
        this.searchedTabList = dataTab?.content;
        this.checkIsShowWarningAgain();
        $("#manage-tab-view").modal("show");
      } catch (error) {
        console.log("manage-tab-view:::show:::error", error);
      }
    },
    search: function () {
      console.log("search");
    ...
    sort: function (type) {
      console.log("type");
      if (type === "standard") {
        this.standardSortType = this.standardSortType === "asc" ? "desc" : "asc";
        ...
        this.requestByMeSortType = this.requestByMeSortType === "asc" ? "desc" : "asc";
    ...
    async deleteTab() {
      ...
      // NEW API
      try {
        const tabDeleteId = this.tabDelete.id;
        await axios.delete(`${API_BASE_TAB_STP}/${this.objectType}/${tabDeleteId}`);
        this.tabDelete = {};
        this.deleteTabAlert = false;
        this.showToastAlert({
          title: "Success!",
    ...
    async refreshPage() {
      ...
      try {
        ...
        console.log("manage-tab-view:::refreshPage:::error", error);
      }
    },
