<template>
  <div class="new-feature-content" id="tooling-table-layout">
    <div class="tooling-table-layout">
      <div class="bottom-content">
        ...
        <div class="content-header">
          New Tooling table layout
        </div>
        <div class="content-body">
          <div class="desc-item">
            Search bar area was shortened and action buttons were grouped together to make more room for the Tooling table below.
          </div>
        </div>
  ...
  methods: {
    ...
    calculatePosition() {
      ...
      if (firstCheckboxOffset) {
        let element = document.getElementById("tooling-table-layout");
        element.style.margin = `${firstCheckboxOffset.y + 26}px ${firstCheckboxOffset.x + 88}px`;
        let position = {
          x: firstCheckboxOffset.x - 12,
          y: firstCheckboxOffset.y - 98
        };
        this.$emit('calculate-position-done', position, firstCheckboxOffset.width + 24,  firstCheckboxOffset.height + 97);
      } else {
...
.tooling-table-layout .top-content {
  background-color: #FFF;
  display: inline-block;
  font-size: 15px;
}
.tooling-table-layout .bottom-content {
  background: #5a4ab9;
  color: #FFF;
  margin-top: 2.5rem;
