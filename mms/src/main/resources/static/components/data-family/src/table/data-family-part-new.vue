                ...
                >
                  ...
                  >
                    ...
                    <td
                      ...
                      class="text-left"
                      v-else-if="column.field === 'projectName'"
                    >
                      {{ result.productName }}
                    </td>
                    <td
                    ...
                    <td
                      :key="index + 'moldCount'"
                      class="text-left"
                      v-else-if="column.field === 'moldCount'"
                    >
                      <a
                        href="javascript:;"
                        @click="directLink('TOOLING', { partId: result.id })"
                      >
                        {{ formatNumber(result.moldCount) }}
                      </a>
                    </td>
                    <td
                      :key="index + 'activeMoldCount'"
                      class="text-left"
                      v-else-if="column.field === 'activeMoldCount'"
                    >
                      <a
                        href="javascript:;"
                        @click="
                        ...
                        "
                      >
                        {{ formatNumber(result.activeMoldCount) }}
                      </a>
                    </td>
                    <td
                      :key="index + 'idleMoldCount'"
                      class="text-left"
                      v-else-if="column.field === 'idleMoldCount'"
                    >
                      <a
                        href="javascript:;"
                        @click="
                        ...
                        "
                      >
                        {{ formatNumber(result.idleMoldCount) }}
                      </a>
                    </td>
                    <td
                      :key="index + 'inactiveMoldCount'"
                      class="text-left"
                      v-else-if="column.field === 'inactiveMoldCount'"
                    >
                      <a
                        href="javascript:;"
                        @click="
                        ...
                        "
                      >
                        {{ formatNumber(result.inactiveMoldCount) }}
                      </a>
                    </td>
                    <td
                      :key="index + 'disconMoldCount'"
                      class="text-left"
                      v-else-if="column.field === 'disconMoldCount'"
                    >
                      <a
                        href="javascript:;"
                        @click="
                        ...
                        "
                      >
                        {{ formatNumber(result.disconMoldCount) }}
                      </a>
                    </td>
                    <td
                      :key="index + 'producedQuantity'"
                      class="text-left"
                      v-else-if="column.field === 'producedQuantity'"
                    >
                      {{ formatNumber(result.producedQuantity) }}
                    ...
                    >
                      ...
                      <div>
                        <span class="font-size-11-2">{{
                          result.size ? getUnitName(result.sizeUnit) : ""
                        }}</span>
                      </div>
                    </td>
                    <td
                    ...
                    >
                      <template v-if="result.machineList">
                        <base-drop-count
                          :is-hyper-link="true"
                    ...
                    >
                      {{ customFieldValuePart(result, column.label) }}
                    </td>
                    <td
...
module.exports = {
  components: {
    ...
    "tab-view-modal": httpVueLoader("/components/tab-view-part-modal-new.vue"),
  ...
  methods: {
    async handleSubmitNote(data) {
      ...
      const { filterCode, query, tabName, timeScale, timeValue } =
        this.requestParam;
      let listId = [];
      ...
      try {
        await axios.post(`${API_BASE_PAR_STP}/note-batch?${param}`, data);
      } catch (error) {
        console.log(error);
      }
    },
    handleChangeDate(date) {
      ...
      this.requestParam.timeValue = `${date.fromTitle.replaceAll("-", "")}`;
      this.paging(1);
    },
    ...
    async removeFromTab() {
      try {
        ...
        await axios.delete(`${API_BASE_PAR_STP}/tab-items-batch?${param}`);
        this.moveSuccess({
          title: "Success!",
          content: `Your data has been removed from ${this.currentActiveTab.name} tab`,
        });
      } catch (error) {
        console.log("data-family-part:::removeFromTab:::error", error);
      }
    },
    moveSuccess(toast) {
      this.getTabsByCurrentUser();
    ...
    async showTab(tab) {
      try {
        tab.shown = true;
        if (tab.defaultTab) {
          await axios.put(`${API_BASE_TAB_STP}/PART/names/${tab.name}/shown`);
        } else {
          await axios.put(`${API_BASE_TAB_STP}/PART/${tab.id}/shown`);
        }
      } catch (error) {
        console.log("devices-part-new:::showTab:::error", error);
      }
    },
    async closeTab(tab) {
      try {
        ...
        if (tab.defaultTab) {
          await axios.put(`${API_BASE_TAB_STP}/PART/names/${tab.name}/hidden`);
        } else {
          await axios.put(`${API_BASE_TAB_STP}/PART/${tab.id}/hidden`);
        }
      } catch (error) {
        console.log("devices-part-new:::closeTab:::error", error);
      }
    },
    closeToastAlert() {
    ...
    async moveToTab(tab) {
      ...
      try {
        await axios.put(`${API_BASE_PAR_STP}/move-tab-items-batch?${param}`);
        this.moveSuccess({
          title: "Success!",
...
*/
        {
          label: this.resources["number_of_machines"],
          field: "machineCount",
          sortable: true,
        },
      ];
    ...
    async showChart(part, tab) {
      const child = this.$refs.chartPart;
      const { data } = await axios.get(`/api/parts/${part.id}`);
      const partData = { ...data, ...part };
      const isUsingNewNoteBatchApi = true;
      if (child != null) {
        child.showChart(
          partData,
          "QUANTITY",
    ...
    async saveBatch(action) {
      ...
      try {
        await axios.put(`${API_BASE_PAR_STP}/${action}-batch?${param}`);
      } catch (error) {
        console.error(error);
      } finally {
        this.listChecked = [];
    ...
    async paging(pageNumber) {
      ...
      console.log(`data-family-part:::param:::`, param);
      ...
      try {
        const { data: partData } = await axios.get(
          API_BASE_PAR_STP + "?" + param,
          {
            cancelToken: this.cancelToken.token,
          }
        );
        this.total = partData.totalElements;
        this.results = partData.content;
        this.pagination = Common.getPagingData(partData);
        ...
        this.listSelectedTabs = this.listSelectedTabs.map((tab) => {
          const total =
            partData.tabs.find((partTab) => partTab.tabName === tab.name)
              ?.totalElements || 0;
          return {
            ...tab,
            total,
    ...
    async exportPart() {
      ...
      try {
        ...
        console.log("data-family-part:::exportPart:::error:::", error);
        this.isStopedExport = true;
      }
    },
    ...
    async getTabsByCurrentUser(isCallAtMounted) {
      try {
        const { data } = await axios.get(`${API_BASE_TAB_STP}/PART`);
        this.listSelectedTabs = data.content;
        if (isCallAtMounted) {
          this.setNewActiveTab();
        }
      } catch (error) {
        console.log("devices-part-new:::getTabsByCurrentUser:::error", error);
      }
    },
    openManageTabView() {
      this.visibleMoreAction = false;
    ...
    customFieldValuePart(object, customFieldLabel) {
      try {
