          ...
          <div class="data-table-tabs d-flex justify-space-between">
            ...
            >
              ...
              >
                <a
                  class="nav-link"
                  :class="{ active: checkStatusParam(tabItem.name) }"
            ...
            <tbody class="op-list" style="display: none">
              <tr v-for="(result, index, id) in results" :id="result.id">
                ...
                <template v-for="column in displayColumns">
                  ...
                  >
                    <span class="font-size-14">{{ result.designedShot }}</span>
                    <div>
                      <span class="font-size-11-2">{{
          ...
          <div class="modal-body">
            <div class="modal-body-content">
              <div class="warning-title">
                ...
                <p v-else>
                  Disable
                  {{ total === listAllIds.length ? total : listChecked.length }}
                  toolings.
                </p>
              </div>
...
const API_BASE_TOL_STP = `/api/common/tol-stp`;
...
module.exports = {
  ...
  methods: {
    ...
    convertCodeToName(tabItem) {
      ...
      if (tabItem.isDefaultTab) {
        if (tabName === "DELETED") return "Disabled";
        return (
          tabName.charAt(0).toUpperCase() +
          tabName.slice(1).toLocaleLowerCase().replace("_", " ")
        );
      }
      return this.truncateText(tabName, 20);
    },
    ...
    checkStatusParam(tabName) {
      /**
       * Condition 1: tab All
       * Condition 2: tab DIGITAL, NON_DIGITAL and DISPOSED
       * Condition 3: other tabs
       */
      if (
        tabName === "All" &&
        ...
        return true;
      } else if (this.requestParam.tabType === tabName) {
        return true;
      } else {
        return this.requestParam.status === tabName && tabName !== "All";
      }
    },
    closeCreateWorkOrderModal() {
      this.workorderModalVisible = false;
    ...
    async saveBatch(idArr, boolean) {
      ...
      try {
        const response = await axios.post(
          "/api/molds/change-status-in-batch",
          param
        );
        this.paging(1);
        this.getTabsByCurrentUser();
    ...
    ratioMaintenanceColor(result) {
      if (result.moldMaintenance.maintenanceStatus === "UPCOMING")
        return "bg-warning";
      else return "bg-danger";
    },
    ...
    tab(tab) {
      ...
      if (["DIGITAL", "NON_DIGITAL"].includes(tab.name)) {
        const isDigitalTabNotFocusable =
          tab.name === "DIGITAL" &&
          !this.listSelectedTabs.some((i) => i.name === "DIGITAL");
        const isNonDigitalTabNotFocusable =
          tab.name === "NON_DIGITAL" &&
          !this.listSelectedTabs.some((i) => i.name === "NON_DIGITAL");
        console.log("isDigitalTabNotFocusable", isDigitalTabNotFocusable);
        console.log("isNonDigitalTabNotFocusable", isNonDigitalTabNotFocusable);
        if (isDigitalTabNotFocusable || isNonDigitalTabNotFocusable) {
          this.requestParam.status = "All";
          this.requestParam.tabType = tab.name;
        }
      } else if ("DISPOSED" === tab.name) {
        this.requestParam.tabType = tab.name;
      }

      console.log("tab:tabType|status", {
        tabType: this.requestParam.tabType,
    ...
    convertParamsInPaging(params) {
      const paramsClone = structuredClone(params);
      Object.entries(paramsClone).forEach(([key, value]) => {
        const isEmptyValue = !value;
        const isEmptyArray = Array.isArray(value) && value?.length === 0;
        if (isEmptyValue || isEmptyArray) delete paramsClone[key];
      });
      const searchParams = new URLSearchParams(paramsClone);

      const { equipmentStatus, operatingStatus, status, tabType } =
        this.requestParam;
      console.log("@convertParamsInPaging:status", status);
      console.log("@convertParamsInPaging:tabType", tabType);

      searchParams.append("isToolingScreen", "true");
      if (operatingStatus === "ALL") searchParams.delete("operatingStatus");
      if (equipmentStatus === "ALL") searchParams.delete("equipmentStatus");
      // const specialTabs =
      if (["DIGITAL", "NON_DIGITAL"].includes(status)) {
        searchParams.set("status", "All");
        searchParams.set("tabType", status);
      }
      if (this.counterId || this.partId) {
        searchParams.append(
          "where",
          `counter${this.counterId},part${this.partId}`
        );
      } else {
        searchParams.append(
          "where",
          `op${this.secondCompare},${this.firstCompare}${this.firstCaviti},${this.thirdCompare}${this.secondCaviti}`
        );
      }
      if (this.currentActiveTab.id) {
        searchParams.append("tabId", this.currentActiveTab.id);
      }
      return searchParams;
    },
    async paging(pageNumber) {
      ...
      const url = new URL(Page.API_BASE, location.origin);
      const params = this.convertParamsInPaging(this.requestParam);
      url.search = params;
      ...
      try {
        ...
        const currentTabIndex = this.listSelectedTabs.findIndex((item) => {
          if (this.currentActiveTab.id) {
            return item.id === this.currentActiveTab.id;
          }
          return item.name === this.currentActiveTab.name;
        });
    ...
    async fetchAllListIds() {
      try {
        var param = Common.param({
          ...this.requestParam,
          tabId: this?.currentActiveTab?.id || "",
    ...
    captureQuery() {
      ...
      if (params?.active) {
        this.digitalactive = params.active;
      ...
      // TODO: REMOVE WHEN OLD DASHBOARD REMOVE
      if (params?.dashboardRedirected) {
        ...
        if (params?.operatingStatus) {
          this.requestParam.operatingStatus = params.operatingStatus;
        }
        if (params?.locationName) {
          this.requestParam.locationName = params.locationName;
        }
      }
      if (params?.partCode) {
      ...
      if (params?.moldStatusList) {
        this.requestParam.moldStatusList = params.moldStatusList.split(",");
      }
      // from which page
      if (params?.from) {
  ...
  computed: {
    ...
    enableBatchDisableAction() {
      return (
        this.listChecked.length >= 1 && this.requestParam.status != "DELETED"
      );
    },
