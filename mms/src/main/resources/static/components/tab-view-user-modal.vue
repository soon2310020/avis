          ...
          <div class="container content-group">
            <div class="content-group-header">
              <div class="row">
                ...
                <!--                  <p v-text="resources['email_address']"></p>-->
                <!--                  <span class="__icon-sort" :class="[sortField === 'email' && sortType ==='asc' ? 'desc' : 'asc']" @click="sort('email')"></span>-->
                <!--                </div>-->
                <div class="col-md-3 tab-group-title">
                  ...
                  <span
                    class="__icon-sort"
                    :class="[
                      sortField === 'company.name' && sortType === 'asc'
                        ? 'desc'
                        : 'asc',
                    ]"
                    @click="sort('company.name')"
                  ></span>
                </div>
                <div class="col-md-3 tab-group-title">
                  ...
                  <span
                    class="__icon-sort"
                    :class="[
                      sortField === 'enabled' && sortType === 'asc'
                        ? 'desc'
                        : 'asc',
                    ]"
                    @click="sort('enabled')"
            ...
            <div class="content-group-wrapper">
              ...
              >
                <div class="col-md-3">
                  <div>
                    <span v-if="data.name && data.name.length > 17">
                    ...
                    <span
                      ...
                      v-text="resources['admin']"
                    ></span>
                    <span
                      v-if="data.ssoId != null"
                      class="label label-warning"
                      v-text="resources['sso']"
                    ></span>
                  </div>

                  <div class="small text-muted font-size-11-2">
                    <span v-text="resources['new']"></span> |
                    <span v-text="resources['registered'] + ' on'"></span>
                    {{ formatToDate(data.createdAt) }}
                  </div>
                </div>
                <div class="col-md-3">
                  <div v-if="data.company != null">
                    {{ data.company.name }}
                    <div class="small text-muted font-size-11-2">
                ...
                <div class="col-md-3 d-flex align-items-center">
                  ...
                  >
                  <span
                    v-if="!data.requested && data.enabled"
                    class="label label-success"
...
module.exports = {
  ...
  methods: {
    ...
    saveTab() {
      ...
      axios.post("/api/tab-table/save-tab-table-user", params).then((res) => {
        this.handleClose();
        if (isCreate) {
    ...
    async getListDataSelectedByTab(isDisable = false, companyType) {
      const res = await axios.get(
        `/api/users?tabId=${
          !this.tab.id ? "" : this.tab.id
        }&id=&query=&status=${
          isDisable ? "disabled" : "active"
        }&page=1&companyType=${companyType}&pageType=${this.pageType}&size=9999`
      );
    ...
    async getAllDataList() {
      const res = await axios.get(
        `/api/users?tabId=&id=&query=${
          this.requestParam.query || ""
        }&status=active&sort=${this.sortData}&page=1&pageType=${
          this.pageType
    ...
    async getAllDataListSearching(pageNumber) {
      ...
      const res = await axios.get(
        `/api/users?tabId=&id=&query=${this.requestParam.query}&status=active&sort=${this.sortData}&page=${this.requestParam.page}&pageType=${this.pageType}&isModalSelected=true`
      );
    ...
    async showCreateTabView(type, tab, companyType) {
      this.clearData();
      ...
      if (type === "EDIT") {
        ...
        this.tab.id = tab.id;
        this.getListDataSelectedByTab(tab.name === "Disabled", companyType);
      } else {
