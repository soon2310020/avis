<template>
  <!-- <div> -->
  <a-modal
    style="position: relative; z-index: 6000"
    :visible="visible"
    :closable="false"
    ...
    <div class="custom-modal-body" style="padding: 16px 24px">
      <div class="custom-select-date-picker">
        <div class="container-date-pick">
          <!--DATE COMPONENT HERE! -->
          <div v-show="selectedFrequency.includes('HOURLY')">
            <hourly-block
              :current-date="currentDate"
              :date-range="dateRange"
              :is-visible="selectedFrequency == 'HOURLY'"
          ...
          <daily-block
            v-show="
              ...
              selectedFrequency.includes('EVERY_SHOT') ||
          ...
          <div
            class="infor-block"
            :style="selectedFrequency == 'EVERY_SHOT' ? 'display: none' : ''"
          >
            <!-- <div
                style="
                  display: flex;
...
module.exports = {
  ...
  components: {
    "frequency-button": httpVueLoader(
      "/components/@base/date-picker/frequency-button.vue"
    ),
    "date-title": httpVueLoader("/components/@base/date-picker/date-title.vue"),
    ...
    "yearly-block": httpVueLoader(
      "/components/@base/date-picker/yearly-block.vue"
    ),
    "hourly-block": httpVueLoader(
      "/components/@base/date-picker/hourly-popup-content.vue"
    ),
  },
  data() {
    return {
  ...
  computed: {
    ...
    getDateBarStatus() {
      console.log("getDateBarStatus", this.selectedFrequency);
      if (
        this.selectedFrequency.includes("RANGE") ||
  ...
  watch: {
    "selectedButton.isRange": function (newValue) {
      console.log("watch selectedButton.isRange", newValue);
    },
  },
  methods: {
    closeDatePicker() {
    ...
    getDefaultData(frequency, dateData) {
      console.log("getDefaultData", frequency, this.selectedButton);
      let date = dateData;
      this.selectedFrequency = frequency;
      const searchBtn = this.selectOption.find(
        (item) => item.type === frequency
      );
      if (searchBtn) {
        this.selectedButton = Object.assign({}, this.selectedButton, searchBtn);
      } else {
        this.selectedButton = Object.assign(
          {},
          this.selectedButton,
          this.selectOption[0]
        );
      ...
      if (date) {
        ...
        let childName = "";
        if (frequency.includes("DAILY") || frequency.includes("EVERY_SHOT")) {
        ...
        if (child != null) {
          console.log("before setDefault", frequency, this.selectedButton);
          child.setDefault(newDate);
        }
      }
    },
    showDatePicker(frequency, date) {
      console.log("showDatePicker 1", frequency, date);
      this.getDefaultData(frequency, date);
      this.visible = true;
      const datePickerRef = document.getElementById("date-picker-modal");
      console.log("showDatePicker", this.dateRange);
      console.log("showDatePicker 2", datePickerRef);
    },
    ...
    clearData() {
      if (this.selectedFrequency == "HOURLY") {
        this.currentDate = {
          from: new Date(),
          to: new Date(),
    ...
    submitDate() {
      ...
      dateData.hour = { ...this.hour };
      console.log("before change date", dateData);
