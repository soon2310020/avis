                    ...
                    >
                      ...
                      >
                        <span
                          ...
                          v-if="result.uptimeStatus === 'OUTSIDE_L1'"
                          v-text="resources['outside_l1']"
                        >
                        </span>
                        <span
                          class="label label-danger"
                          v-if="result.uptimeStatus === 'OUTSIDE_L2'"
...
<script>
const ALERT_UPTIME_API_BASE = "/api/production/alr-utm";
...
module.exports = {
  ...
  data() {
    return {
      ...
      sortType: {
        ...
        GOALS: "baseEfficiency",
        UPTIME: "efficiency",
        OP: "operatingStatus",
        VARIANCE: "variance",
        DATE: "creationDateTime",
        STATUS: "alertStatus",
        CONFIRM_DATE: "confirmedDateTime",
      },
      currentSortType: "id",
      isDesc: true,
      isLoading: false,
  ...
  methods: {
    ...
    async paging(pageNumber) {
      ...
      const url = new URL(ALERT_UPTIME_API_BASE, location.origin);
      url.search = params;
      try {
        const { data: uptimeAlertData } = await axios.get(url, {
          cancelToken: this.cancelToken.token,
        });
        this.isLoading = false;
        this.total = uptimeAlertData.totalElements;
        this.results = uptimeAlertData.content;
        this.tabs = uptimeAlertData.tabs;
        this.pagination = Common.getPagingData(uptimeAlertData);
        ...
        this.isLoading = false;
        console.log("mold-efficiency:::paging:::error:::", error);
      }
    },
    ...
    showSystemNoteModal(selectedItem) {
      ...
      if (child) {
        ...
        if (this.listAllIds.length === this.total) {
          ...
          console.log("single select in history tab");
          this.handleShowNoteSystem({ id: selectedItem.moldId });
        }
      }
    },
    ...
    captureQuery() {
      ...
      if (params?.tab) {
        this.tab(TAB_CONFIG.find((tab) => tab.status === params.tab));
      } else {
        TAB_CONFIG.find((tab) => tab.status === this.requestParam.status); // default
      }
    },
    ...
    async handleSubmitNote(data) {
      ...
      try {
        await axios.post(`${ALERT_UPTIME_API_BASE}/note-batch?${param}`, data);
      } catch (error) {
        console.log("mold-efficency:::handleSubmitNote:::error", error);
      }
    },
  },
  created() {
