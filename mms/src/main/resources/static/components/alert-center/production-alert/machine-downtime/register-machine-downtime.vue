...
module.exports = {
  ...
  methods: {
    ...
    show(machineDowntimeAlert, type) {
      ...
      let machineDowntimeAlertData = {
        ...machineDowntimeAlert,
        startTime: moment(
          machineDowntimeAlert.startTime,
          formatTemplate
        ).unix(),
        endTime: moment(machineDowntimeAlert.endTime, formatTemplate).unix(),
        downtimeStatus: machineDowntimeAlert.alertStatus,
      };
      axios
        .get(
          `/api/machine-downtime-alert/reasons/${machineDowntimeAlertData.id}`
        )
        .then((response) => {
          this.itemData = Object.assign(
            {},
            this.itemData,
            machineDowntimeAlertData
          );
          this.modelType = type;
          ...
          this.reasonNote = "";
          this.planDowntimeType =
            machineDowntimeAlertData.downtimeType === "PLANNED_DOWNTIME"
              ? "PLANNED"
              : "UNPLANNED";
