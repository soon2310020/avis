                  ...
                  <template>
                    ...
                    >
                      ...
                      >
                        ...
                        >
                          <div v-if="result.previousLocationId">
                            <div class="font-size-14">
                              <a
                                ...
                                "
                              >
                                {{ result.previousLocationName }}
                              </a>
                            </div>
                            <span class="font-size-11-2">
                              {{ result.previousLocationCode }}
                            </span>
                          </div>
                        </td>
                        <td
...
<script>
const ALERT_RELOCATION_API_BASE = "/api/asset/alr-rlo";
const TAB_CONFIG = [
  {
    name: "Alert",
    key: "alert",
  ...
  methods: {
    ...
    setAllColumnList() {
      ...
      this.allColumnList = [
        ...
        {
          label: this.resources["relocated_from"],
          field: "previousLocation",
          default: true,
          sortable: true,
          sortField: "previousLocationName",
          defaultSelected: true,
          defaultPosition: 2,
        },
        {
          label: this.resources["relocated_to"],
          field: "location",
          default: true,
          sortable: true,
      ...
      if (this.allColumnList) {
        this.allColumnList.forEach((c) => {
          if (c.field == "confirmedAtShow") {
            c.hiddenInToggle = tab.status == "confirmed" ? false : true;
          }
        });
      }
    ...
    async paging(pageNumber) {
      ...
      const url = new URL(ALERT_RELOCATION_API_BASE, location.origin);
      url.search = params;
      this.isLoading = true;

      try {
        const { data: relocationAlertData } = await axios.get(url, {
          cancelToken: this.cancelToken.token,
        });
        this.isLoading = false;
        this.total = relocationAlertData.totalElements;
        this.results = relocationAlertData.content;
        this.tabs = relocationAlertData.tabs;
        this.pagination = Common.getPagingData(relocationAlertData);
        ...
        this.isLoading = false;
        console.log("mold-locations:::paging:::error:::", error);
      } finally {
        this.isLoading = false;
      }
    },
    ...
    async handleSubmitNote(data) {
      ...
      try {
        await axios.post(
          `${ALERT_RELOCATION_API_BASE}/note-batch?${param}`,
          data
        );
      } catch (error) {
        console.log("mold-locations:::handleSubmitNote:::error", error);
      }
    },
  },
  created() {
