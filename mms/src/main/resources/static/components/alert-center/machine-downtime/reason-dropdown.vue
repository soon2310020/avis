<template>
  <div v-if="reasons.length === 1">
    <div v-if="reasons[0].codeData.title.length > 20" v-bind:data-tooltip-text="reasons[0].codeData.title"> {{ replaceLongtext(reasons[0].codeData.title, 20) }} </div>
    <div v-else> {{ reasons[0].codeData.title}} </div>
  </div>
  <div
      ...
      <span class="one-line">
        ...
        >
          <div
              ...
              :key="index"
          >
            <p v-if="reason.codeData.title.length > 20" class="part-name-drop-down one-line" v-bind:data-tooltip-text="reason.codeData.title">
              {{ replaceLongtext(reason.codeData.title, 20) }}
            </p>
            <p class="part-name-drop-down one-line">
              {{reason.codeData.title}}
            </p>
          </div>
        </div>
      </span>
