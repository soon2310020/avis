<template>
  <div style="position: relative">
    <input
      ...
      v-if="show"
      :checked="checked"
      @click.prevent="showSelectAllOptions = true"
    />
    <base-popover
      :is-visible="showSelectAllOptions"
      class="select-all-popover"
    ...
    >
      <ul class="select-all-options">
        <li class="select-all-item" @click="handleSelectAll(ACTION_ALL.PAGE)">
          Select <b>{{ count }}</b> {{ count > 1 ? unit[1] : unit[0] }} on this
          page
        </li>
        ...
        <li
          ...
          @click="handleSelectAll(ACTION_ALL.DESELECT)"
        >
          Deselect all
        </li>
      </ul>
