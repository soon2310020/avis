    ...
    <template>
      ...
      >
        <base-button
          level="primary"
          type="reject"
          @click="handleDeclineDataRequest"
          >{{ resources["decline_data_request"] }}
        </base-button>
      </div>
    </template>
  </base-dialog>
...
module.exports = {
  ...
  setup(props, ctx) {
    ...
    // METHOD //
    const handleDeclineDataRequest = () => {
      ctx.emit("decline-data-request", reason.value);
    };
    ...
    const getWarningMessage = computed(() => {
      return props.resources[
        "declined_request_will_be_sent_to_history_log_as_default"
      ];
    });

    const getPlaceHolder = computed(() => {
      return props.resources[
        "enter_the_reason_for_declining_the_data_request_here"
      ];
    });
