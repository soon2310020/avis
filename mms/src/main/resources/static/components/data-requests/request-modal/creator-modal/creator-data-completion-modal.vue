        ...
        <div class="steps-zone">
          ...
          <div v-if="visible" class="steps-content">
            ...
            <form-type
              ...
              :resources="resources"
              :types="getTypesArray"
              @select-type="handleChangeDataRequested"
...
const DEFAULT_STEP = [
  ...
  },
  {
    ...
    btnTitleKey: "create_data_completion_request",
    value: 4,
    checked: false,
  },
];
...
module.exports = {
  ...
  components: {
    "form-type": httpVueLoader(
      "/components/data-requests/request-modal/creator-modal/data-completion-form/type.vue"
    ),
    "form-assign": httpVueLoader(
      "/components/data-requests/request-modal/creator-modal/data-completion-form/assign.vue"
    ),
    "form-schedule": httpVueLoader(
      "/components/data-requests/request-modal/creator-modal/data-completion-form/schedule.vue"
    ),
  ...
  setup(props, ctx) {
    ...
    const getTypesArray = computed(() => {
      ...
      const result = typeKeys.map((item) => {
        const newItem = {
          ...
          ...types.value[item],
          id: item,
          // ...{ key: item }, ...types.value[item]
        };
        newItem.title = `${props.resources[newItem.multipleLabelKey]} (${
          newItem.value
        })`;
        return newItem;
      });
    ...
    watch(
      ...
      async (newVal) => {
        if (newVal) {
          ...
          await getDataTypeNumbers();
          if (props.modalType === "CREATE") {
            const newId = await generateRequestId();
            request.value.requestId = newId;
            listSteps.value[3].btnTitleKey = "create_data_completion_request";
            requestDate.value = {
              from: new Date(),
              to: new Date(),
            ...
            if (props.modalType === "EDIT") {
              listSteps.value[3].btnTitleKey = "edit_data_completion_request";
            }
            if (props.modalType === "REOPEN") {
              listSteps.value[3].btnTitleKey =
                "re_open_data_completion_request";
            }
            ...
            if (props.selectedDataRequest.isSelectedCompany) {
              handleChangeDataRequested("COMPANY");
            }
            if (props.selectedDataRequest.isSelectedLocation) {
              handleChangeDataRequested("PLANT");
            }
            if (props.selectedDataRequest.isSelectedMachine) {
              handleChangeDataRequested("MACHINE");
            }
            if (props.selectedDataRequest.isSelectedMold) {
              handleChangeDataRequested("TOOLING");
            }
            if (props.selectedDataRequest.isSelectedPart) {
              handleChangeDataRequested("PART");
            }
          }
          console.log("props", props.selectedDataRequest);

          console.log("show modal", newVal);
        } else {
          clearData();
        }
      }
    ...
    const navigate = (type) => {
      console.log("navigate", type, request.value);
      if (type === "next") {
    ...
    const setUpPayload = () => {
      ...
      payload.isSelectedCompany = types.value["COMPANY"].checked;
      payload.isSelectedLocation = types.value["PLANT"].checked;
      payload.isSelectedPart = types.value["PART"].checked;
      payload.isSelectedMold = types.value["TOOLING"].checked;
      payload.isSelectedMachine = types.value["MACHINE"].checked;
    ...
    const handleChangeDataRequested = (key) => {
      // types.value[key].checked = !types.value[key].checked
      console.warn("handleSelectDataType", key);
      Vue.set(types.value, key, {
        ...types.value[key],
        checked: !types.value[key].checked,
      });
      // console.log('handleSelectDataType', types.value)
    };

    const handleSelectAll = (isSelectAll) => {
      const keys = ["COMPANY", "PLANT", "PART", "TOOLING", "MACHINE"];
      if (isSelectAll) {
        keys.forEach((element) => {
          types.value[element].checked = true;
        });
      } else {
        keys.forEach((element) => {
          types.value[element].checked = false;
        });
      }
    };
    ...
    const clearData = () => {
      currentStep.value = _.cloneDeep({ ...DEFAULT_STEP[0] });
      listSteps.value = [...DEFAULT_STEP];
      request.value = _.cloneDeep({ ...INIT_REQUEST });
      timeOption.value = [...DEFAULT_TIME_OPTION];
      timeRange.value = _.cloneDeep({ ...DEFAULT_TIME_RANGE });
    ...
    const getDataTypeNumbers = async () => {
      try {
        ...
        types.value["COMPANY"].value = numbers["companyNumber"];
        types.value["PLANT"].value = numbers["locationNumber"];
        types.value["MACHINE"].value = numbers["machineNumber"];
        types.value["TOOLING"].value = numbers["moldNumber"];
        types.value["PART"].value = numbers["partNumber"];
      } catch (error) {
        console.log(error);
      }
    };
