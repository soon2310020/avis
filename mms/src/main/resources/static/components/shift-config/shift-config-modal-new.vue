    ...
    <div v-show="!isLoading" class="modal-body">
      <div class="modal-body__header__infor">
        <div class="modal-body__title">
          ...
          <span v-if="locations.length == 1">{{ location.locationTitle }}</span>
          <location-selection
            v-else
      ...
      <div class="d-flex hour-picker-container">
        <div class="single-container">
          <div class="title__name" style="margin-bottom: 10px">Day Start</div>
          <hour-picker
      ...
      <div class="shift-config-modal__footer">
        <primary-button
          ...
          @on-active="handleSaveShiftConfig()"
...
module.exports = {
  ...
  components: {
    ...
    "location-selection": httpVueLoader(
      "/components/shift-config/location-selection.vue"
    ),
  },
  created() {
    this.dayList = [
  ...
  computed: {
    getModalTitle() {
      return "Plants Shift Configuration";
    },
    isEnalbeSubmit() {
      if (this.selectedShiftData.hourShiftData) {
        const check = this.selectedShiftData.hourShiftData.filter(
          (item) =>
            (item.start == "" && item.end == "") ||
            (item.start != "" && item.end != "")
        );
    ...
    getEndHourRangePicker() {
      let result = `${
        +this.dayStart.hour + 1 > 9
          ? +this.dayStart.hour + 1
          : +this.dayStart.hour + 1 < 0
          ? "00"
          : "0" + (+this.dayStart.hour + 1)
      }00`;
  ...
  methods: {
    ...
    async showModal() {
      this.locations = this.shiftConfigLocations;
      this.location = this.shiftConfigLocations[0];
      await this.initData();
    },
    ...
    getHourRange1(shiftName, index, type) {
      ...
      if (type == "start") {
        ...
        if (end !== "") {
          ...
          max = `${
            +this.selectedShiftData.end.substr(0, 2) - 1 > 9
              ? +this.selectedShiftData.end.substr(0, 2) - 1
              : +this.selectedShiftData.end.substr(0, 2) - 1 < 0
              ? "23"
              : "0" + (+this.selectedShiftData.end.substr(0, 2) - 1)
          }00`;
        }
      } else if (type == "end") {
        ...
        if (start !== "") {
          min =
            +start.substr(0, 2) == 23
              ? "0000"
              : `${
                  +start.substr(0, 2) + 1 > 9
                    ? +start.substr(0, 2) + 1
                    : "0" + (+start.substr(0, 2) + 1)
                }00`;
        } else {
          min = `${
            +this.selectedShiftData.start.substr(0, 2) + 1 > 9
              ? +this.selectedShiftData.start.substr(0, 2) + 1
              : "0" + (+this.selectedShiftData.start.substr(0, 2) + 1)
          }00`;
        }
        isAddDay = true;
      }
    ...
    handleChangeHour(shiftName, data, index) {
      ...
      let startHour = `${
        data.start.hour == "" || data.start.minute == "" ? "" : data.start.hour
      }${
      ...
      let endHour = `${
        data.end.hour == "" || data.end.minute == "" ? "" : data.end.hour
      }${data.end.hour == "" || data.end.minute == "" ? "" : data.end.minute}`;
      currentShiftData[shiftName][index].start = `${startHour}`;
      currentShiftData[shiftName][index].end = `${endHour}`;
      let indexOfTotal =
        this.getShiftSize * (+shiftName.replaceAll("shift", "") - 1) + index;
    ...
    resetShiftConfig(currentShiftData) {
      const end = this.dayEnd.isAddDay
        ? +this.dayEnd.hour + 24
        : +this.dayEnd.hour;
      const start = +this.dayStart.hour;
    ...
    initHourPicker() {
      ...
      console.log(
        "initHourPicker",
        this.selectedShiftData.start,
        this.selectedShiftData.end,
        this.dayStart,
        this.dayEnd
      );
