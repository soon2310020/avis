          ...
          <div class="d-flex align-items-center justify-content-between">
            ...
            <div class="d-flex align-items-baseline">
              <div class="add-tooling-wrapper">
                ...
                >
                  <common-select
                    :style="{ position: 'static', width: '100%!important' }"
            ...
            <div class="content-group-header">
              <div class="row">
                <div class="col-md-3 tab-group-title active">
                  <p v-text="resources['counter_id']"></p>
                  <span
                    class="__icon-sort"
                    :class="[
                ...
                <div class="col-md-3 tab-group-title">
                  ...
                  <span
                    class="__icon-sort"
                    :class="[
                      sortField === 'mold.equipmentCode' && sortType === 'asc'
                        ? 'desc'
                        : 'asc',
                    ]"
                    @click="sort('mold.equipmentCode')"
                  ></span>
                </div>
                <div class="col-md-3 tab-group-title">
              ...
              >
                <div class="col-md-3">
                  ...
                  <div class="small text-muted font-size-11-2">
                    <span v-text="resources['updated_at']"></span>
                    {{ formatToDate(result.updatedAt) }}
                  </div>
                </div>
                <div class="col-md-3">
                  <div>{{ result.mold?.equipmentCode }}</div>
                </div>
                <div class="col-md-3">
                  <div v-if="result.companyName">
                    <a
                      class="font-size-14"
                      href="#"
                      @click.prevent="showCompanyDetailsById(result.companyId)"
                    >
                      {{ result.companyName }}
                    </a>
                    <div class="small text-muted font-size-11-2">
                ...
                <div class="col-md-3 action-column">
                  <div
                    v-if="checkDataSelected(result.id)"
                    class="d-flex align-items-center h-100 justify-content-start"
                    @click="addDataToTab(result.id)"
                  >
                  ...
                  <div
                    v-else
                    class="d-flex align-items-center h-100 justify-content-start"
                    @click="removeDataFromTab(result.id)"
                  >
...
module.exports = {
  ...
  methods: {
    ...
    showCounterDetails: function (data) {
      this.$emit("show-counter-details", data);
    },
    ...
    onChangeMethodAdd(value) {
      if (value.name === "page") {
        ...
        this.tab.addItemIdList = this.allDataList
          .map((i) => i.id)
          .filter((e) => e);
      }
      this.$forceUpdate();
    ...
    async saveTab() {
      ...
      // axios
      //   .post("/api/tab-table/save-tab-table-counter", params)
      //   .then((res) => {
      //     this.handleClose();
      //     if (isCreate) {
      ...
      try {
        if (isCreate) {
          const { data: sensorTabData } = await axios.post(
            `${API_BASE_TAB_STP}/COUNTER/one`,
            dataBody
          );
          this.handleClose();
          this.$emit("create-success", sensorTabData, {
            title: "Success!",
            content: "Your tab view has been created",
          });
        } else {
          const { data: sensorTabData } = await axios.put(
            `${API_BASE_TAB_STP}/COUNTER/${tab.id}`,
            dataBody
          );
          this.$emit("edit-success", sensorTabData, {
            title: "Success!",
            content: "Your tab view has been edited",
          });
        }
      } catch (error) {
        console.log("tab-view-counter-modal:::saveTab:::error", error);
      }
    },
    validForm() {
      if (!this.tab.name) {
    ...
    async getListDataSelectedByTab(tabName) {
      ...
      const res = await axios.get(`${API_BASE_SSR_STP}?${param}`);
    ...
    async getAllDataList() {
      // const res = await axios.get(
      //   `/api/counters?tabId=&id=&query=&status=all&sort=${this.sortData}&page=1&pageType=${this.pageType}&isModalSelected=true&size=99999`
      // );
      const requestParam = {
        counterStatus: "",
        toolingStatus: "",
    ...
    async getAllDataListSearching(pageNumber) {
      ...
      // const res = await axios.get(
      //   `/api/counters?tabId=&id=&query=${this.requestParam.query}&status=active&sort=${this.sortData}&page=${this.requestParam.page}&pageType=${this.pageType}&isModalSelected=true`
      // );
    ...
    async showCreateTabView(type, tab, tabName) {
      this.clearData();
